...
(async()=>{
 ...
 assert.equal(await page.locator('#hmRows tr').count(),5);
 assert(!(await page.locator('#hmRows').innerText()).includes('證書'));
 assert(!(await page.locator('#hmDetail').innerText()).includes('介面統一稱'));
 await page.locator('[name="detailSupplement"][value="haccp30"]').check();
 await page.locator('#detailLicense').selectOption('dietitian');
 let proofs=await page.locator('.document-list').innerText();assert(proofs.includes('營養師證書')&&proofs.includes('30小時')&&!proofs.includes('60小時'));
 await page.locator('[name="detailRoute"][value="a4-ordinary-exam"]').check();
 proofs=await page.locator('.document-list').innerText();assert(proofs.includes('3年以上')&&!proofs.includes('畢業證書'));
 await page.locator('[name="detailSupplement"][value="haccp60"]').check();
 assert.equal(await page.locator('#detailLicense').count(),0);
 proofs=await page.locator('.document-list').innerText();assert(proofs.includes('60小時')&&!proofs.includes('營養師證書')&&!proofs.includes('30小時'));
 await page.locator('[name="detailRoute"][value="a4-degree"]').check();
 await page.screenshot({path:path.join(out,'desktop-meat.png'),fullPage:true});
 await page.locator('#hmMajor').fill('食品科學系');await page.locator('#hmSearch').click();await page.locator('#hmRows button').first().click();assert((await page.locator('#hmDetail').innerText()).includes('專科以上'));assert((await page.locator('#hmDetail').innerText()).includes('專科以上符合科系資格畢業證書'));
